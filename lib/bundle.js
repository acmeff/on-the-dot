...
var Game = function () {
  function Game() {
    ...
    this.buttons();
    setTimeout(function () {
      return _this.demo = new Demo(_this.target);
    }, 1000);
    this.count = 0;
  }
  ...
  _createClass(Game, [{
    ...
    value: function buttons() {
      ...
      remove.onclick = function () {
        var card = _this2.stack.removeCard();
        card.drawDots();
      };

      var go = document.getElementById('gen-stack');
      go.onclick = function () {
        var cards = _this2.stack.cards;
        _this2.stack.clearCanvas();
        _this2.stack = new Stack(_this2.stackCtx, _this2.stackCanvas, cards);
        _this2.compare();
      };
    ...
    value: function compare() {
      ...
      if (diff < 2000) {
        ...
        setTimeout(function () {
          _this3.target.drawNext();
          _this3.clearStack();
          _this3.match.classList.add("hidden");
          _this3.match.classList.remove('appear');
        }, 1500);
      } else {
        this.notMatch.classList.remove('hidden');
        this.notMatch.classList.add('appear');
        setTimeout(function () {
          _this3.notMatch.classList.add("hidden");
          _this3.notMatch.classList.remove('appear');
        }, 1500);
      }
    }
  }]);
...
var Target = function (_Card) {
  ...
  _createClass(Target, [{
    ...
    value: function round1() {
      ...
      this.drawDot([191.25, 165.75], "green");
      this.drawDot([242.25, 191.25], "green");
      this.drawDot([191.25, 89.25], "green");
      this.drawDot([242.25, 63.75], "green");
      this.drawDot([191.25, 242.25], "red");
      this.drawDot([165.75, 191.25], "red");
      this.drawDot([89.25, 191.25], "red");
      this.drawDot([63.75, 242.25], "red");
      this.drawDot([12.75, 191.25], "purple");
      this.drawDot([63.75, 165.75], "purple");
      this.drawDot([63.75, 89.25], "purple");
      this.drawDot([12.75, 63.75], "purple");
      this.imgData = this.ctx.getImageData(0, 0, 300, 300).data;
    }
  }]);
